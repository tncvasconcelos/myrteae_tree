...
wd <- "~/Desktop/Myrteae_codes/"

#############################
###### 1. MANTEL TESTS ######
# set working directory
setwd(paste(wd, "2_mantel/18.feb.20", sep=""))
...
#### ALL 52 ####
list.files(pattern="_all52_tree") -> tree
sapply(tree, read.nexus, simplify=F) -> trees
sub("_all52_tree", "", tree) -> labs
...
results_final_all52<-c()
n0 <- c()
for(u in 1:length(result_mantel)){
  x0<- round(as.numeric(result_mantel[[u]][4]) ,2)
  results_final_all52[u] <- paste(str_split(unlist(result_mantel[[u]][1]), "_")[[1]][1], 
                                  x0, sep=" ")
  n0[u] <- str_split(unlist(result_mantel[[u]][1]), "_")[[1]][1]
}
names(results_final_all52) <- n0

#### Eugeniinae ####

list.files(pattern="_eugeniinae_tree") -> tree
sapply(tree[-5], read.nexus, simplify=F) -> trees
sub("_eugeniinae_tree", "", tree) -> labs
...
setwd(paste(wd, "2_median_bs/18.feb.20", sep=""))

#### ALL 52 ####
# bootstrap 
list.files(pattern="_all52_bs.txt") -> bs
sapply(bs, readChar, nchars=1e9, simplify=F) -> bs_all
r <-gsub("[\\(\\)]", "", regmatches(bs_all[[1]], gregexpr("\\[.*?\\]", bs_all[[1]]))[[1]])

results_median_all52 <- data.frame(matrix(NA, nrow = length(r), ncol = length(bs_all)))
for(i in 1:length(bs_all)){
  r0<-gsub("[\\(\\)]", "", regmatches(bs_all[[i]], gregexpr("\\[.*?\\]", bs_all[[i]]))[[1]])
  r0<-as.numeric(str_extract_all(r0, "[0-9]+"))
  results_median_all52[,i]<-r0
  colnames(results_median_all52)[i]<-paste(bs[i])
}

mns <- colMedians(as.matrix(results_median_all52))
results_median_all52 <- results_median_all52[,order(mns)]
names(results_median_all52) <- sub("_all52_bs.txt", "", names(results_median_all52))
boxplot(results_median_all52, col= colors, las=1)

#### EUGENIINAE ####
list.files(pattern="_eugeniinae_bs.txt") -> bs
sapply(bs, readChar, nchars=1e9, simplify=F) -> bs_all
r <-gsub("[\\(\\)]", "", regmatches(bs_all[[1]], gregexpr("\\[.*?\\]", bs_all[[1]]))[[1]])
...
names(results_median_eug) <- sub("_eugeniinae_bs.txt", "", names(results_median_eug))
...
setwd(paste(wd, "2_treespace/18.feb.20", sep=""))

#### ALL 52 ####
#### treespace 1 - markers ####
list.files(pattern="_all52_trees.txt") -> files
sub("_all52_trees.txt", "", files[-c(2,6,7)]) -> labs
...
names(all.trees) -> qualifier_all52_markers
unlist(lapply(strsplit(qualifier_all52_markers, "-"), "[", 1)) -> qualifier_all52_markers
# Root trees
outgroup = "Eucalyptus.perriniana_Lucas.283"
lapply(all.trees, root, outgroup, resolve.root=T) -> all.trees
.compressTipLabel(all.trees) -> all.trees
# Run treespace
res_all52_markers <- treespace(all.trees, nf=3)
...
all.trees
names(trees) -> names(all.trees)
names(all.trees) -> qualifier_all52_part
unlist(lapply(strsplit(qualifier_all52_part, "-"), "[", 1)) -> qualifier_all52_part
# Root trees
outgroup = "Eucalyptus.perriniana_Lucas.283"
lapply(all.trees, root, outgroup, resolve.root=T) -> all.trees
.compressTipLabel(all.trees) -> all.trees
# Run treespace
res_all52_part <- treespace(all.trees, nf=3)


#### EUGENIINAE ####
#### treespace 1 - markers ####
list.files(pattern="_eugeniinae_tree.txt") -> files
sub("_eugeniinae_tree.txt", "", files[-c(1,3,4)]) -> labs
...
#### Myrciinae ####
#### treespace 1 - markers ####
list.files(pattern="_myrciinae_tree.txt") -> files
sub("_myrciinae_tree.txt", "", files[-c(1,4,5)]) -> labs
...
# all 52
boxplot(results_median_all52,col=colors[order(match(names(colors),names(results_median_all52)))],
        las=1, cex.axis=0.75, cex.lab=0.75,
        xlab=paste(results_final_all52[order(match(names(results_final_all52),
                                                 names(results_median_all52)))],collapse = "; "))
title(main="All markers (52 tips)")

s.class(res_all52_markers$pco$li, fac=as.factor(qualifier_all52_markers), cellipse = 0, 
        cstar=1, label=NULL, pch=19,cgrid=1, cpoint = 0.5, 
        col=colors[levels(as.factor(qualifier_all52_markers))])
s.chull(res_all52_markers$pco$li, fac=as.factor(qualifier_all52_markers), add.plot = T, 
        clabel=1.2, col=colors[levels(as.factor(qualifier_all52_markers))], 
        label=names(colors[levels(as.factor(qualifier_all52_markers))]), 
        optchull=1)

s.class(res_all52_part$pco$li, fac=as.factor(qualifier_all52_part), cellipse = 0, cstar=1, 
        label=NULL, pch=19,cgrid=1, cpoint = 0.5, 
        col=colors[levels(as.factor(qualifier_all52_part))])
s.chull(res_all52_part$pco$li, fac=as.factor(qualifier_all52_part), 
        add.plot = T, clabel=1.2, col=colors[levels(as.factor(qualifier_all52_part))], 
        label=names(colors[levels(as.factor(qualifier_all52_part))]), 
        optchull=1)
...
title(main="Myrciinae markers (295 tips)")
...
title(main="Eugeniinae markers (329 tips)")
